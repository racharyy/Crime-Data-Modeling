...
#                 self.ivar = 1. / np.random.rand(5)
#                 self.y=0.2

#         def logprob(self,x):
#                 logp = -0.5 * np.sum(self.ivar * self.y*x**2)
#                 grad = -self.ivar * x*self.y
#                 return logp, grad


#         def class_hmc(self):
                
#                 samples = hmc(self.logprob, x0=np.random.randn(5),  n_samples=1000)  
#                 print samples
# cl = basic()
# cl.class_hmc()

# def logprob(x, ivar,y):
#     logp = -0.5 * np.sum(ivar * y*x**2)
#     grad = -ivar * x*y
#     return logp, grad
# # run the sampler
...
# y=0.2
# samples = hmc(logprob, x0=np.random.randn(5), args=(ivar,y), n_samples=1000)
# print samples
